...
while True:
    ...
    def find_closest_color(image, masks, colors):
        closest_y = -1
        closest_color = None

    for mask, color_name in zip(masks, colors):
        contours, _ = cv2.findContours(mask, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
        for contour in contours:
            M = cv2.moments(contour)
            if M["m00"] > 200:  # Adjust threshold to ignore noise
                cY = int(M["m01"] / M["m00"])  # Y-coordinate of the centroid

                if cY > closest_y:  # Check if this block is closer (lower in the image)
                    closest_y = cY
                    closest_color = color_name

    # Store the line information
    if closest_color and closest_line_info is None:
        closest_line_info = closest_color
        print(f"Closest line detected: {closest_line_info}")
